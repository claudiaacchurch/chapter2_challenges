...
def remove_nones_from_list(my_list):
    new_list = [item for item in my_list if item != None]
    return new_list
...
#   Returns: ["a", "b", "c", "d"]
def add_to_list(given_list, given_element):
    given_list.append(given_element)
    return given_list
...
def new_band_member(my_dict):
    current_dict = {"vocalist": "miss piggy", "lead_guitar": "scooter"}
    current_dict.update(my_dict)
    return current_dict 
...
def all_keys(dictionary):
    new_list = [x for x in dictionary.keys()]
    return new_list
...
def remove_nones_from_dictionary(dictionary):
    new_dict = {}
    new_dict = {k: v for k,v in dictionary.items() if k != None and v != None}
    return new_dict
...
def touch_in(station, time):
    new_dict = {"entrypoint": station, "time": time}
    return new_dict