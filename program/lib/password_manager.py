...
class PasswordManager():
    ...
    def add(self, service_name, password):
        symbols = ['!', '@', '$', '%', '&']
        if len(password) > 7 and any(symbol in password for symbol in symbols):
            password_dict[service_name] = password 
        return None
    ...
    def list_services(self):
        dict_list = [service for service in password_dict.keys()]
        return dict_list 
